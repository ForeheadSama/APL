...
def write_syntax_analysis(message):
    with open("syntax_analysis.txt", "a") as f:
        f.write(message + "\n")
...
def write_ast(node, file, level=0):
    """Write AST node to file with enhanced type and value information"""
    indent = "  " * level
    ...
    if isinstance(node, dict):
        ...
        # Add declared type info
        if 'declared_type' in node:
            file.write(f" declared_type:{node['declared_type']}")
            
        # Add actual type (for expressions)
        if 'expr_type' in node:
            file.write(f" expr_type:{node['expr_type']}")
           
        # Add value for literals with type information
        if 'value' in node and not isinstance(node['value'], (dict, list)):
            file.write(f" = {node['value']}")
            if 'value_type' in node:
                file.write(f" (value_type:{node['value_type']})")
           
        # Add operator for binary operations
        if 'op' in node:
            file.write(f" [{node['op']}]")
            
        # Add expected vs actual parameter counts for function calls
        if node['type'] == 'function_call' and 'expected_param_count' in node:
            file.write(f" expected_params:{node['expected_param_count']} actual_params:{len(node.get('arguments', []))}")
            
        # Add return information for functions
        if node['type'] == 'function_def' and 'return_type' in node:
            file.write(f" return_type:{node['return_type']}")
            
        # Add expected return type for return statements
        if node['type'] == 'return' and 'expected_type' in node:
            file.write(f" expected_type:{node['expected_type']}")
            if 'actual_type' in node:
                file.write(f" actual_type:{node['actual_type']}")
           
        file.write("\n")
       
        # Process children in specific order with enhanced context
        for key in ['statements', 'condition', 'then_block', 'else_block',
                   'body', 'left', 'right', 'params', 'arguments', 'value']:
